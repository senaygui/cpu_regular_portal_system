...
class AcademicCalendarPdfGenerator
  ...
  def header(pdf)
    pdf.text 'CPU Business and Information Technology College Portal Academic Calendar', size: 24, style: :bold,
                                                                                         align: :center
    pdf.move_down 20
  ...
  def semesters_table(pdf)
    ...
    @semesters.each do |semester|
      table_data << [semester.semester, semester.starting_date.strftime('%b %d, %Y'),
                     semester.ending_date.strftime('%b %d, %Y')]
    end
