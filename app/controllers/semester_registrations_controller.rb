class SemesterRegistrationsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @semester_registration.update(semester_registration_params)
        format.html do
          redirect_to invoice_path(@semester_registration.invoices.last.id),
  ...
  def destroy
    ...
    respond_to do |format|
      format.html do
        redirect_to semester_registrations_url, notice: 'semester_registration was successfully destroyed.'
      end
  ...
  def download_pdf
    ...
    pdf.table(table_data, header: true, row_colors: %w[F0F0F0 FFFFFF], cell_style: { borders: %i[top bottom] }) do
