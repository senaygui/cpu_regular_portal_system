class NonGcStudentReport < Prawn::Document
    ...
    def each_data_in_table(students)
      [
        ['No.', 'ID No', 'First Name', 'Middle Name', 'Last Name', 'Sex', 'Birth DateD/M/Y', 'Citizenship',
         'Admission Date', 'Program']
      ] + students.map.with_index do |student, index|
        [index + 1, student.student_id, student.first_name.capitalize, student.middle_name.capitalize,
         student.last_name.capitalize, student.gender, student.date_of_birth.strftime('%d-%B-%Y'), student.nationality, student.admission_date.strftime('%d-%B-%Y'), student.program.program_name]
      end
    end
end
