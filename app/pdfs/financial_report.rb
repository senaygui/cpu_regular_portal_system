class FinancialReport < Prawn::Document
  ...
  def header_footer
    repeat :all do
      bounding_box [bounds.left, bounds.top], width: bounds.width do
        ...
        text 'CPU Business and Information Technology College Registrar Portal', align: :center, size: 25
      ...
      bounding_box [bounds.left, bounds.bottom + 40], width: bounds.width do
        ...
        stroke_horizontal_rule
        move_down(5)
        text 'CPU Business and Information Technology College Portal', size: 16, align: :center
  ...
  def each_data_in_table(students)
    [
      ['No', 'Student Name', 'Departement', 'Year', 'Semester', 'Total Price', 'Remaining Amount', 'Actaul Payment',
       'Mode of Payment']
    ] + students.map.with_index do |smr, index|
      [index + 1, smr.student.name.full, smr.department.department_name, smr.year, smr.semester, smr.total_price,
       smr.remaining_amount, smr.actual_payment, smr.mode_of_payment]
    end
  end
  ...
  def preview_table(data)
    [
      ...
      ['Cumulative'] + get_cumulative(get_previous_total(data.student, data.semester), data.total_credit_hour,
                                      data.total_grade_point, data.cgpa)
